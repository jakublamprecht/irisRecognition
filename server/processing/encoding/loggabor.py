## Copied from https://github.com/AntiAegis/Iris-Recognition/blob/master/python/fnc/encode.py
...
def encode(polar_array, noise_array, minWaveLength, mult, sigmaOnf):
    ...
    # Switching from color range 0-255 to bit range 0/1
    noise_array = noise_array.astype(bool)
    ...
    # If amplitude is close to zero then phase data is not useful,noi
    # so mark off in the noise mask
    H3 = np.abs(eleFilt) < 0.0001
    # Invert so that bitwise and works
    H3 = np.invert(H3)
    ...
    for i in range(length):
        ...
        # Create noise mask
        # Changed to and, since my mask is inverted (0 is the masked bit, 1 is the good bit)
        mask[:, ja] = noise_array[:, i] & H3[:, i]
        mask[:, ja + 1] = noise_array[:, i] & H3[:, i]

    # Converting mask template to previewable image
    template = template.astype(int) * 255
